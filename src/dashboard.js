...
app.post('/api/tickets/:ticketId/close', requireDashboardAuth, async (req, res) => {
  try {
    ...
    // Resolve ticket first, so we can fall back to its guildId
...
app.post('/api/tickets/:ticketId/reply', requireDashboardAuth, async (req, res) => {
  try {
    ...
    if (!ticketId || !content) {
      return res.status(400).json({ ok: false, error: 'ticketId and content are required' });
    }

    // First resolve ticket by ID only
