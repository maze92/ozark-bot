...
app.post('/api/mod/warn', requireDashboardAuth, rateLimit({ windowMs: 60_000, max: 30, keyPrefix: 'rl:mod:warn:' }), async (req, res) => {
  try {
...
app.post('/api/mod/unmute', requireDashboardAuth, rateLimit({ windowMs: 60_000, max: 30, keyPrefix: 'rl:mod:unmute:' }), async (req, res) => {
  try {
      const client = _getClient();
    const body = req.body || {};
    const parseResult = ModWarnSchema.safeParse(body);
...
app.post('/api/mod/reset-trust', requireDashboardAuth, rateLimit({ windowMs: 60_000, max: 10, keyPrefix: 'rl:mod:resetTrust:' }), async (req, res) => {
  try {
      const client = _getClient();
    const body = req.body || {};
    const parseResult = ModWarnSchema.safeParse(body);
...
app.post('/api/mod/remove-infraction', requireDashboardAuth, rateLimit({ windowMs: 60_000, max: 20, keyPrefix: 'rl:mod:removeInfraction:' }), async (req, res) => {
  try {
      const client = _getClient();
...
app.get('/api/mod/overview', requireDashboardAuth, rateLimit({ windowMs: 20_000, max: 30, keyPrefix: 'rl:mod:overview:' }), async (req, res) => {
  try {
